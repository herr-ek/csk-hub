...
describe("email verification", () => {
  test("sends an email-verification OTP", async () => {
    ...
    await expect(sendEmailVerificationOtp("user@example.com")).resolves.toEqual({ success: true })
    expect(sendVerificationOtp).toHaveBeenCalledWith({ email: "user@example.com", type: "email-verification" })
  })

  test("verifies an email with its OTP", async () => {
    verifyEmail.mockResolvedValue({ error: null })

    await expect(verifyEmailOtp("user@example.com", "123456")).resolves.toEqual({ success: true })
    expect(verifyEmail).toHaveBeenCalledWith({ email: "user@example.com", otp: "123456" })
  })
