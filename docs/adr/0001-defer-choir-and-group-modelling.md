...
membership as flat: every User belongs to the organisation, and information goes to
everyone. We considered adopting Better Auth's `organization` plugin up front, with
Organization = Choir and `member` records belonging to several at once, and deliberately chose not
...
identity. The real cost lands in application code: every query that lists Users or
