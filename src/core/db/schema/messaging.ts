...
export const directConversation = pgTable(
  ...
  (table) => [
    ...
    // Better Auth IDs use ASCII characters, whose code-unit order matches PostgreSQL's C collation.
    check(
      "directConversation_member_pair_order_check",
      sql`${table.firstMemberId} COLLATE "C" < ${table.secondMemberId} COLLATE "C"`
    )
  ]
)
