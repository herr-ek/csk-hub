# Inactive Users ride on the admin plugin's ban fields
...
Inactive User (ADR-0002) *is* a `user` row with `banned = true`. `banReason` and
`banExpires` are left null: withdrawing access is permanent until an Admin reverses it,
never time-limited.

## Consequences

The plugin already refuses to create a session for a banned user, so "an Inactive User
